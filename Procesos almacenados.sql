...
CREATE PROCEDURE obtener_reportes_alumno_con_id(IN p_username VARCHAR(50))
BEGIN
    SELECT 
        r.id_reporte,
        r.fecha,
        ...
        r.reporte
    FROM Reportes r
    INNER JOIN Docentes de ON r.id_docente = de.id
    INNER JOIN Usuarios d ON de.id = d.id  -- Para obtener nombre del docente
    INNER JOIN Docente_Materias dm ON r.docente_materia_id = dm.id
    INNER JOIN Materias m ON dm.materia_id = m.id
    INNER JOIN Alumnos a ON r.id_alumno = a.id
    INNER JOIN Usuarios u ON a.id = u.id  -- Para filtrar por username
    WHERE u.user_ = p_username
    ORDER BY r.fecha DESC;
END//

CREATE PROCEDURE obtener_username_por_id_alumno(IN p_id_alumno INT)
BEGIN
    SELECT user_ 
    FROM Usuarios u
...
CREATE PROCEDURE eliminar_reporte_por_id(IN p_id_reporte INT)
BEGIN
...
create procedure corte_actual_porMateria(in id_docente int)
begin
...
begin
	UPDATE Docente_Materias SET corte_actual = corte WHERE id = id_docente;
end//
