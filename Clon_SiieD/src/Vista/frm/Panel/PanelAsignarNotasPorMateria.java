...
public class PanelAsignarNotasPorMateria extends JPanel {
    ...
    // Corte actual persistente en BD
    private int corteActualDB = 1;
    ...
    private final DefaultTableModel modelo = new DefaultTableModel(
        ...
        public boolean isCellEditable(int row, int column) {
            if (column >= 2 && column <= 4) { 
                ...
                if (valorNota instanceof Number) {
                    return ((Number) valorNota).doubleValue() == 0.0;
                } else if (valorNota instanceof String) {
                    try {
    ...
    private void cargarTablaConEdicion() {
        ...
        // Cargar corte actual desde BD
        try {
            corteActualDB = basedatos.obtenerCorteActual(docenteMateriaId);
            if (corteActualDB < 1 || corteActualDB > 3) {
                corteActualDB = 1;
            }
        } catch (Exception e) {
            corteActualDB = 1;
        ...
        try {
            ...
            e.printStackTrace(); // IMPORTANTE: Para ver el error exacto
        }
    }
    ...
    private void guardarNotas() {
        ...
        for (Integer row : filasEditadas) {
            try {
                String valor = modelo.getValueAt(row, corteActualDB + 1).toString().trim();
                if (!valor.isEmpty()) {
                    ...
                    if (nota < 0.0 || nota > 5.0) {
                        ...
                        tabla.setColumnSelectionInterval(corteActualDB + 1, corteActualDB + 1);
                        return;
                    }
                }
        ...
        for (Integer row : filasEditadas) {
            ...
            double nota = Double.parseDouble(valor);
            batchData.add(new Object[]{alumnoId, corteActualDB, dmId, nota});
        }
    ...
    private void moveToNextEditableCell(int currentRow, int currentCol) {
        ...
        int opcion = JOptionPane.showConfirmDialog(this, 
            "¿Terminó de ingresar notas del Corte " + corteActualDB + "?\n\n" +
